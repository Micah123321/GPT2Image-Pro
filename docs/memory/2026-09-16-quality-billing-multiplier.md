...
## 创作页报价联动（2026-09-17 追加，同日修正口径）

`listImageBackendGroupOptions` 附带每分组的 `qualityMultipliers`（组内启用
成员 API+账号的 quality_billing 各档最大值）。**预估不折入质量倍率**——具体
倍率取决于调度最终命中的成员，预估阶段无法预知（曾按各档上限折入，因口径
不准被否）；仅当预测车道（mixed 合并父组与预测子组）存在质量计价配置时，
公式旁注明「未计入质量倍率」，实际扣费以命中成员解析为准。

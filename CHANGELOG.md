...
报价的计算口径展示；调度侧修复同优先级换号预算用尽后的换档行为。

### 新增

- **API 出站模型映射**：自定义 API 与池 API 可配置「本站型号 → 上游型号」规则表，支持按请求 quality 分档命中并提供 Adobe GPT Image / Gemini 渠道预设；命中后可同时改写出站 quality。测活与后台表单同步支持。
- **质量计价倍率**：池 API 与 ChatGPT 账号（web/codex 车道）均可按质量档位（auto/low/medium/high/xhigh/max）配置计费倍率；倍率折入总倍率（分组/成员 × 模型族 × 质量）单点贯通预扣、Chat/Agent 轮次、成功结算与退款，账单元数据留痕质量分量与请求质量便于对账。未配置档位按 1 倍计，`xhigh`/`max` 未单独配置时回退 `high` 档。
- **报价与明细公式化**：历史记录明细改为展示倍率前的基础生图/审核附加积分，倍率行更名为「计费倍率」；创作页四处预估费用（文生图/图生图/对话轮次/批量单张）展示完整计算公式「（基础 + 审核）× 分组倍率」。质量倍率取决于调度最终命中的成员，预估不折入；预测车道内存在质量计价配置时公式旁注明「未计入质量倍率」。

### 修复

- **换号调度**：同优先级换号预算用尽后正确进入 priority 更大的下一档并钉住已用尽档位；无图输出上限按整次请求累计不随换档清零。

### 升级说明

- **本版本包含数据库迁移 0048**：`image_backend_api` 与 `image_backend_account` 新增 `quality_billing` json 列（`ADD COLUMN IF NOT EXISTS`，幂等可重复执行）。升级前先执行迁移再切换实例；未配置质量倍率时计费行为与旧版完全一致。
- 质量倍率为每后端独立配置，内部同步链路（Sub2API 同步、注册机产号）不会覆盖库内已有配置。
